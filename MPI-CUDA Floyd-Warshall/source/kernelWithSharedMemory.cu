...
__global__ void kernelWithSharedMemory(float *A, float *row_k, int *n, int *k);
...
{ 
  ...
  id_i_k = (*k) + y * gridDim.x * blockDim.x;   //id for A(i,k)       
  a_i_k = A[id_i_k];                                       
...
void executeKernelWithSharedMemory(int n, int number_of_rows , int world_rank, float *testArray)
{
  ...
  for(k = 0; k < n; k++){
    ...
    kernelWithSharedMemory<<<dimGrid,dimBlock>>>(d_A, d_row_k, d_n, d_k);
    //cudaThreadSynchronize();

    //Copy result back to host 
    cudaMemcpy(testArray, d_A, n*n/world_size*sizeof(float), cudaMemcpyDeviceToHost);

    //Write to the graphics card the new values.There is no need to do that as the values are already stored in the device 
    /*if(k != n-1)
      cudaMemcpy(d_A, testArray, n*n/world_size*sizeof(float), cudaMemcpyHostToDevice);*/
  }
